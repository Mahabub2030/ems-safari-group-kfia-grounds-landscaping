...
// Renamed on import so it doesn't collide with the `documents` state
// variable declared below (previously both had the same name, and the
// state declaration itself was commented out — that's what caused the
// `formData is not defined` / documents-never-updating bugs).
import { documents as initialDocuments } from "../../data/Documentsdata";
...
// ----------------------------------------------------------------------
//
// IMPORTANT: Define these types ONCE and import them everywhere they're
// used. If you already have a shared types file (e.g. `./types.ts` or
// `./Documents.tsx`), move this block there and import from both files
// instead of duplicating it — that was the root cause of the earlier
// TS2367 error (two different `DocumentType` types being compared).

export type DocumentType = "Passport" | "Iqama" | "CV" | "Job Offer" | "Other";
...
export default function EmployeeDocuments() {
 // Single source of truth for the documents list. `initialDocuments`
 // (from Documentsdata) seeds it; `setDocuments` is what all the
 // add/edit/delete handlers and the fetch effect actually call.
 ...
 // Form state used by the Edit / Add dialogs. This was referenced via
 // `formData` / `setFormData` throughout the component but never
 // declared — that's the `formData is not defined` ReferenceError.
 ...
 useEffect(() => {
  ...
  async function loadDocuments() {
   ...
   try {
    ...
    const data: EmployeeDocument[] = await res.json();
    if (!cancelled) {
     setDocuments(data);
    }
   } catch (err) {
    if (!cancelled) {
     ...
     // Keep showing whatever is already in `documents` as a fallback.
    }
   } finally {
    if (!cancelled) {
     setIsLoading(false);
 ...
 const handleDownload = (
  ...
  if (url && url !== "#") {
   ...
   const dummyContent = `=========================================\n${filename.toUpperCase()}\n=========================================\nDocument ID : ${docId || "DOC-FILE"
    }\nCategory    : ${type || "Attachment"
 ...
 const handleDeleteConfirm = () => {
  ...
  setDocuments((prev) => prev.filter((d) => d.docId !== selectedDoc.docId));
 ...
 const handleSaveEdit = () => {
  ...
  setDocuments((prev) =>
   prev.map((d) =>
    d.docId === selectedDoc.docId
     ? ({ ...d, ...formData } as EmployeeDocument)
     : d,
   ),
  );
  setIsEditOpen(false);
 ...
 const handleSaveAdd = () => {
  const newDoc: EmployeeDocument = {
   docId: `DOC-${Date.now()}`,
   docName: formData.docName || "Untitled Document",
 ...
 return (
  <div className="container mx-auto p-4 md:p-8 max-w-[96rem] space-y-8">
    ...
    <div>
     ...
     {fetchError && (
      <p className="text-xs text-rose-600 mt-1">
       Couldn't load documents from the server ({fetchError}) — showing
       local data.
      </p>
     )}
    </div>
   ...
   {/* Document Preview Modal (Drive / PDF / Image) */}
   ...
   <Dialog open={isViewOpen} onOpenChange={setIsViewOpen}>
    <DialogContent className="max-w-lg">
     <DialogHeader>
      <DialogTitle>{selectedDoc?.docName}</DialogTitle>
     </DialogHeader>
     {selectedDoc && (
      <div className="space-y-2 text-sm">
       <div>
        <strong>Doc ID:</strong> {selectedDoc.docId}
       </div>
       <div>
        <strong>Category:</strong> {selectedDoc.docType}
       </div>
       <div>
       ...
       {selectedDoc.group && (
        <div>
         <strong>Group:</strong> {selectedDoc.group}
        </div>
       )}
       {selectedDoc.iqamaNumber && (
        <div>
         <strong>Iqama #:</strong> {selectedDoc.iqamaNumber}
        </div>
       )}
       {selectedDoc.passportNumber && (
        <div>
         <strong>Passport #:</strong> {selectedDoc.passportNumber}
        </div>
       )}
       <div>
        <strong>Issue Date:</strong> {selectedDoc.issueDate}
       </div>
       {selectedDoc.expiryDate && (
        <div>
         <strong>Expiry Date:</strong> {selectedDoc.expiryDate}
        </div>
       )}
      </div>
     )}
     ...
     <div className="space-y-3">
      <Input
       ...
       onChange={(e: { target: { value: string } }) =>
        setFormData((f) => ({ ...f, docName: e.target.value }))
       }
      />
      <Input
       ...
       onChange={(e: { target: { value: string } }) =>
        setFormData((f) => ({ ...f, employeeName: e.target.value }))
       }
      />
   ...
   <Dialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
    <DialogContent className="max-w-sm">
     ...
     <p className="text-sm text-muted-foreground">
      This will permanently remove "{selectedDoc?.docName}" from the list.
     </p>
